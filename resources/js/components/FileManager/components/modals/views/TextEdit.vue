<template>
  <div>
    <el-row>
      <el-col :span="24">
        <p>{{ selectedItem.basename }}</p>
        <codemirror ref="fmCodeEditor" v-model="code" :options="cmOptions"></codemirror>
      </el-col>
      <el-col :span="24" style="padding:10px;text-align:center">
        <el-button-group>
          <el-button type="primary" v-on:click="updateFile">{{ lang.btn.submit }}</el-button>
          <el-button type="primary" v-on:click="hideModal">{{ lang.btn.cancel }}</el-button>
        </el-button-group>
      </el-col>
    </el-row>
  </div>
